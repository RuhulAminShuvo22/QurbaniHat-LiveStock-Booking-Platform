...
const AllAnimalsPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // রিলেটিভ পাথ ব্যবহার করা হয়েছে যাতে Vercel-এ সমস্যা না হয়
        const res = await fetch("/data.json");
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    // 💰 min price
    if (minPrice) {
      result = result.filter((animal) => animal.price >= Number(minPrice));
    }

    // 💰 max price
    if (maxPrice) {
      result = result.filter((animal) => animal.price <= Number(maxPrice));
    }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 p-4">
        {filteredAnimals.length > 0 ? (
          ...
          <p className="text-red-500 font-semibold">No animals found 😢</p>
        )}
      </div>
    </div>
  );
